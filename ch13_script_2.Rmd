...
# If we divide the US population into 5 income categories and compare people in
# two adjoining buckets, say the 3s and the 4s (since rule of 4 applies best in
# the middle of the distribution), then the richer people are about 5% less
# likely to prefer the Democratic candidate, all else equal, but it might be as
# low as 4% or as high as 7%.
```
...
# Can you write a simple dplr pipe which does, more or less, what the CART is doing, at least for the first node, and then confirms the numbers in the plot?

nes %>% 
  filter(! ideology >= 4) %>% 
  summarise(total = n(), dems = sum(vote == "Democratic")) %>% 
  mutate(per = dems/total)
